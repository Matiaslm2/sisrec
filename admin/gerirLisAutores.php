...
$table->setTitle("Lista de Autores");

//select the table in the datebase
$table->prepareTable("Lista de Autores");
//SELECT `id`, `titulo`, `texto`, `imagens`, `link`, `idCat` FROM `sisrecArtigos` WHERE 1
//list of fields for list, new, edit and import records
$table->setFieldsAtive("id, titulo, texto, imagens, link, idCat",'list');
$table->setFieldsAtive("titulo, texto, imagens, link, idCat", 'new');
$table->setFieldsAtive("titulo, texto, imagens, link, idCat", 'edit');
$table->setFieldsAtive("titulo, texto, imagens, link, idCat", 'csv');
...
$table->setFieldList("idCat",1," SELECT `CodAutor`, `CodLivro` FROM `Autor` ORDER BY `autor`");
