...
$sql = "SELECT `CodLivros`,`ISBN` , `Titulo` FROM `Livros` WHERE `ISBN`!='' and `Capa`='SemCapa.jpg' order by DataPublicacao desc limit 2"; //. $salto;
echo $sql;
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                ...
                //if(!file_exists('imagens/livros/'.$row['ISBN'].'.jpg')){
                    echo "<br>Não  existe";
                    $website = file_get_contents('https://www.goodreads.com/search?q=978-989-690-009-0&qid=aXWvUCelVZ'.$row['ISBN'].'/');
